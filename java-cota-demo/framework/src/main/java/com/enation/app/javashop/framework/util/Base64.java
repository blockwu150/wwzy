...
public class Base64 {
	...
	public static String encodeLong(Long id) throws UnsupportedEncodingException {
		byte b = 0;
		char bb = '0';
		short ss = (short) bb;
		...
		for (int i = 0; i < uidBytes.length; i++) {
			uidBytes[i] = (byte) (id >> (8 * (8 - 1 - i)));
		}
		// uidBytes[0] = (byte)(uid >> (8*7));

		byte[] encodeUid = java.util.Base64.getUrlEncoder().encode(uidBytes);
		String dstUid = new String(encodeUid, "UTF-8");

		return dstUid;
	}
	public static Long decodeToLong(String code) {
		byte[] decodeUid = java.util.Base64.getUrlDecoder().decode(code.getBytes());
		long rsUid = 0L;
