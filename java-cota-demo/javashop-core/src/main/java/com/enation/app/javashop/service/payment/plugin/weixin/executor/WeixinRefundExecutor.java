...
public class WeixinRefundExecutor extends WeixinPuginConfig {
    ...
    public boolean returnPay(RefundBill bill) {
        ...
        params.put("refund_fee", CurrencyUtil.toFen(bill.getRefundPrice()));
        params.put("total_fee", CurrencyUtil.toFen(bill.getTradePrice()));
        ...
        try {
            ...
            if(!file.exists()){
                this.cache.put(REFUND_ERROR_MESSAGE+"_"+bill.getRefundSn(), "找不到证书路径"+config.getP12Path()+"，请联系管理员正确配置");
                return false;
            }
            ...
            if (SUCCESS.equals(returnCode)) {
                ...
                if(SUCCESS.equals(resultCode)){
                    ...
                    this.cache.put(REFUND_ERROR_MESSAGE+"_"+bill.getRefundSn(), failReason);
            ...
            this.cache.put(REFUND_ERROR_MESSAGE+"_"+bill.getRefundSn(), "异常");
            e.printStackTrace();
            return false;
        }
    }
