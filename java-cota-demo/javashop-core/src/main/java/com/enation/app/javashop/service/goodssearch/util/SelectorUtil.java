...
import com.enation.app.javashop.service.goods.util.BrandUrlUtils;
...
import com.enation.app.javashop.model.goodssearch.PropSelector;
...
public class SelectorUtil {
     ...
     * @return
     */
    public static List<SearchSelector> createCatSelector(List<LongTerms.Bucket> categoryBuckets, List<CategoryVO> allCatList, Long catId) {
        String catPid = null;
        if (catId != null) {
//            String[] catar = catId.split(Separator.SEPARATOR_PROP_VLAUE);
        ...
        if (categoryBuckets != null && categoryBuckets.size() > 0) {
            for (Bucket bucket : categoryBuckets) {
                ...
                String catname = "";
                CategoryVO findcat = CatUrlUtils.findCat(allCatList, StringUtil.toLong(categoryId, 0));
                if(findcat == null){
                    continue;
                }
                //没有传递cat参数
                if (catPid == null) {
                    ...
                    CategoryVO parentCat = CatUrlUtils.findCat(allCatList, StringUtil.toLong(path[1], 0));
                    ...
                    CategoryVO cat = CatUrlUtils.findCat(allCatList, StringUtil.toLong(path[0], 0));
    ...
    public static List<SearchSelector> createBrandSelector(List<LongTerms.Bucket> brandBuckets, List<BrandDO> brandList) {
        ...
        if (brandBuckets != null && brandBuckets.size() > 0) {
            for (Bucket bucket : brandBuckets) {
                Long brandid = StringUtil.toLong(bucket.getKey().toString(), 0);
                String brandname = "";
